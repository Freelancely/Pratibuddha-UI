      ...
      this.productService.filterProducts().subscribe((response) => {
        ...
        if (this.status) {
          if (this.status === 'on-sale') {
            this.products = this.products.filter((p) => p.discount !== undefined && p.discount > 0);
          } else if (this.status === 'in-stock') {
            this.products = this.products.filter((p) => p.status === 'in-stock');
          } else if (this.status === 'out-of-stock') {
            this.products = this.products.filter((p) => p.status === 'out-of-stock' || (p.quantity !== undefined && p.quantity === 0));
          }
        }
        ...
        // Price Filter
        this.products = this.products.filter(
          (p) => p.price !== undefined && p.price >= Number(this.minPrice) && p.price <= Number(this.maxPrice)
        );
      });
    });
  }
