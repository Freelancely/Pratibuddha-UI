...
export class UtilsService {
  ...
  handleOpenModal(id: string, item: IProduct) {
    ...
    this.modalId = id;
    this.product = item;
    // Use the first image from productImageUrl if img is not available
    const activeImage = item.img || (item.productImageUrl && item.productImageUrl[0]?.img);
    if (activeImage) {
      this.productService.handleImageActive(activeImage);
    }
